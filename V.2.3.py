...
for i in range(N):
  ball[i]=sphere( pos=vector(0,4,0) , radius=0.8 , color=color.red , mass=20 , velocity=vector(0,-1,0), force=vector(0,0,0) )
...
def computer_force(i):
 ...
 for i in range(N):   
  ...
  ## Corregir el problema de las fuerzas en z ..     
...
## for i in range(N):   
##    delta=-ball[i].radius-ball[i].pos.z+wallU.pos.z+ancho
##    if delta > 0:
##       ball[i].force.z= k*delta
       
## ---------------     
nops=10
U=range(1001) ## este valor sale de hacer nobs/dt+1   
K=range(1001)
T=range(1001)
a=-1
