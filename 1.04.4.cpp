...
int main(void)
{
  ...
  bodies[i].Vx = a/(sqrt(a*a+b*b))*V ;
  bodies[i].Vy = b/(sqrt(a*a+b*b))*V ;
  ...
  for (int it = 0; it < 1000000; ++it) {
    ...
    for (int ji=0; ji<N;++ji){
      su=su+bodies[ji].mass*bodies[ji].Vx*bodies[ji].Vx+bodies[ji].mass*bodies[ji].Vy*bodies[ji].Vy;
    }

    fout << DT*it << "  " << su << std::endl; //---->aqui es donde se imprime los datos para graficar. 
